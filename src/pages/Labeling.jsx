...
const Labeling = () => {
  ...
  // Clave única: ID + nombre — preserva LÁSER NOW 1 y LÁSER NOW 2 (mismo ID, distinto nombre)
  // pero elimina filas genuinamente duplicadas (mismo ID Y mismo nombre desde varias hojas)
  ...
  const filteredData = useMemo(() => {
    return dedupedData.filter(p => {
      ...
      // Con filtro específico: comparación normalizada
      const matchesStatus = status === norm(statusFilter);
  ...
  const selectedPedidos = useMemo(() => {
    return dedupedData.filter(p => selectedIds.includes(getKey(p)));
  }, [dedupedData, selectedIds]);
